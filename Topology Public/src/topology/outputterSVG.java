...
public class outputterSVG {
  ...
  public void drawPolar(double scaleR) {
    ...
    for(int i=0;i<graph.numNodes();i++){
      ...
      circle.setAttribute("id", ""+graph.node(i).id);
      circle.setAttribute("cx", ""+cx);
      if (Math.abs(cx)>latoMassimo) latoMassimo=Math.abs(cx);
      circle.setAttribute("cy", ""+cy);
      if (Math.abs(cy)>latoMassimo) latoMassimo=Math.abs(cy);
      circle.setAttribute("r", ""+raggio);
    ...
    double lato = (latoMassimo+raggio)*1.1;
    String L = String.valueOf(lato);
    String L2 = String.valueOf(2*lato);
    radice.setAttribute("viewBox", "-"+L+" "+"-"+L+" "+L2+" "+L2);
    //==========================================================================
    // Fine xml per Svg
