...
public class Graph implements Runnable {
  ...
  double scale_R=1;
  double r = 10;
  double k_repulsive=1;
  double k_attratctive=1;
  volatile double Kgaussian=0;
  double delta;
  private class node {
    int id;
    String XMLidentification;
    String name;
    String riferimento;
    double angle;
    double delta=0;
    ...
    int indice;
    void setAngle(int p) {
      angle=2*Math.PI*(p%Nodes.size())/Nodes.size();
    }
    double centrality;
    double n_centrality;
  ...
  void addNode(int id, String name, String XMLidentification, String riferimento) {
    node node = new node();
    node.id = id;
    node.name= name;
    node.indice=Nodes.size();
    node.XMLidentification=XMLidentification;
    node.riferimento=riferimento;
    Nodes.add(node);
  }
  ...
  public void run() {
    ...
    for (int source=0; source<numeroRouters; source++) {
      ...
      addNode(source, router.getAttribute("name"), router.getAttribute("id"), router.getAttribute("riferimento"));
    ...
    for (int i=0; i<numeroLinks; i++) {
      ...
      try {
        ...
        String typeLink = link.getAttribute("type");
        addLink(source, target, distance, typeLink);
  ...
  void initialPosition(double deltaCentrality) {
    ...
    for (int i=0; i<daPosizionareArray.size(); i++) {
      ...
      if (!daPosizionare.initialPositioned) {
        daPosizionare.setAngle(posizione);
